...
inductive Subst (Γ : Ctx) : (Γ' : Ctx) → Type
 | nil                                 : Subst Γ .nil
 | cons (γ : Subst Γ Γ') (V : Val Γ A) : Subst Γ (Γ'.cons A)

@[simp]
def Subst.map (f : ∀ {A}, (V : Val Γ A) → Val Γ'' A) : (γ : Subst Γ Γ') → Subst Γ'' Γ'
  | nil      => nil
  | cons γ V => cons (γ.map @f) (f V)
...
def translateSubst : (γ : CBV.Subst Γ Γ') → Modal.Subst Γ Γ'
  | .nil      => .nil
  | .cons γ V => .cons (translateSubst γ) (translateVal V)

@[simp]
def translateState : (S : CBV.State A) → Modal.State A
  | (I, M, O) => (I, translateExp M, O)

def translateExpVal : (V : CBV.Val M) → Modal.Reduces (I, translateExp M, O) (I, .ret (translateVal V), O)
  | .var  x
  | .triv
  | .lam  M
  | .zero   => .refl
  | .inl  V
  | .inr  V
  | .succ V => .trans (.lift .let₁ (translateExpVal V)) (.step .let₂ .refl)
  | .pair V₁ V₂ => .trans (.lift .let₁ (translateExpVal V₁)) (.step .let₂ <| by simp; exact .trans (.lift .let₁ (translateExpVal V₂)) <| .step .let₂ <| by simp; exact .refl)

@[simp]
theorem translateVal_ofNat : translateVal (.ofNat n) = .ofNat (Γ := Γ) n :=
  by induction n <;> simp [*]
...
theorem translateSubst_id (f : ∀ {A}, (x : CBV.Var A Γ) → CBV.Var A Γ') : translateSubst (.ofRenaming (.var <| f ·) .id) = .ofRenaming (.var <| f ·) .id :=
  by induction Γ <;> simp [*]
