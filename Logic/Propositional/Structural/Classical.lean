...
inductive Seq : (Γ : Ctx) → (Δ : Ctx) → Type
  ...
  | notR (v : Mem A.not Δ) (D : Seq (Γ.cons A) Δ) : Seq Γ Δ
  | notL (u : Mem A.not Γ) (D : Seq Γ (Δ.cons A)) : Seq Γ Δ
...
def Seq.rename (γ : Rename Γ Γ') (δ : Rename Δ Δ') : (D : Seq Γ Δ) → Seq Γ' Δ'
  ...
  | trueR v => trueR (δ v)
  | falseL u => falseL (γ u)
  | notR v D => notR (δ v) (D.rename γ.cons δ)
  | notL u D => notL (γ u) (D.rename γ δ.cons)
  | andR v D₁ D₂ => andR (δ v) (D₁.rename γ δ.cons) (D₂.rename γ δ.cons)
  | andL₁ u D => andL₁ (γ u) (D.rename γ.cons δ)
  | andL₂ u D => andL₂ (γ u) (D.rename γ.cons δ)
  | orR₁ v D => orR₁ (δ v) (D.rename γ δ.cons)
  | orR₂ v D => orR₂ (δ v) (D.rename γ δ.cons)
  | orL u D₁ D₂ => orL (γ u) (D₁.rename γ.cons δ) (D₂.rename γ.cons δ)
...
def Seq.cut : (D : Seq Γ (Δ.cons A)) → (E : Seq (Γ.cons A) Δ) → Seq Γ Δ
  ...
  | D@(notR .here D₁), E@(notL .here E₁) => cut (cut (D.rename .id (.cons .weakening)) E₁) (cut D₁ (E.rename (.cons .weakening) .id))
  ...
  | falseL u, _ => falseL u
  | notR (.there v) D, E => notR v (cut D (E.rename (.cons .weakening) .id))
  ...
  | D, notL (.there u) E => notL u (cut (D.rename .id (.cons .weakening)) E)
